...
\node [align=center] at (current page.north) % upper
    {\begin{tikzpicture}[remember picture, overlay]
  \fill[fill=$cover.label-color$] (-10,0) rectangle (10,-0.5);
...
\node [yshift=+6cm] at (current page.center) % title
    {\begin{tikzpicture}[remember picture, overlay]
    \fill[fill=$cover.label-color$] (-10,-5) rectangle (10 ,1);
    \node[align=center] at (0,$if(cover.subtitle)$-1$else$-2$endif$)
        {\resizebox{1.3\linewidth}{!}{\Huge\textbf{\textsc{
        $cover.title$
        }}}};$if(cover.subtitle)$
    \node[align=center] at (0,-3)
        {\resizebox{0.85\linewidth}{!}{\Huge\textbf{\textsc{
            $cover.subtitle$
        }}}};$endif$
...
\node [align=center] at (current page.south) % bottom
    ...
    \node[anchor=south west, align=left] at (-10,0.5)
        {\resizebox{.3\linewidth}{!}
          {\Huge\color{black}
            \textbf{\color{$cover.label-color$}\ccbysa \color{black}}}};
...
\scalerel*{\includegraphics{./icons/doi_icon.pdf}} {O} \href{https://doi.org/$citation.doi$}{$citation.doi$}
