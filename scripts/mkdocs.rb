...
logger.info "Processing markdown files at \"#{markdown_volumes}\" ..."

Dir["#{markdown_volumes}/*.md"].each_with_index do |volume_file, v|
    logger.debug "Processing volume #{v + 1} of #{Dir["#{markdown_volumes}/*.md"].count} ..."
...
logger.info "Copying assets from \"#{markdown_pages}\" ..."
...
logger.info "Finished copying #{Dir["#{markdown_pages}/**/*.md"].count} assets to \"#{docs_markdown}\""
...
logger.info "#{Dir["#{markdown_volumes}/**/*.md"].count} docx files converted, #{Dir["#{docx_dir}/**/*.docx"].count - Dir["#{markdown_volumes}/**/*.md"].count} docx files skipped, #{Dir["#{docs_markdown}/**/*.md"].count} markdown files created, #{Dir["#{markdown_pages}/**/*.md"].count} assets copied, #{Dir["#{yml_files}/**/*.yml"].count} yml files transfered"
