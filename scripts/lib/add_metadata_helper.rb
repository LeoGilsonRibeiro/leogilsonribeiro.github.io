...
def add_metadata(author, editor, title, text, vol_title, path) # rubocop:disable Metrics/AbcSize
  ...
  abnt_ref = "#{author_name}. \"#{title}\". In#{author == editor ? "" : " #{editor_name} (org.)"} <em>#{vol_title}</em>, #{Time.now.strftime("%Y")}. Publicação original: #{text_to_title(publisher)}, #{orig_date}. URL: <a href=\"yml_view_url\">#{yml_view_url}</a>".gsub(/'/, '').gsub(/(^|$)/, "'")
  ...
  yml_header = [yml_delimiter,
                ...
                'print_bib: true',
                "orig_publisher: #{text_to_title(publisher)}",
                "orig_date: #{orig_date}",
                annote,
                "abnt_ref: #{abnt_ref}",
